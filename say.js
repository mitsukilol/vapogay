...
exports.run = async (client, message, args) => {
  ...
  let  devtr_kod = message.guild.members.cache.filter(m => !m.user.bot && m.user.presence.status !== "offline").size //#DevTR - xFalcon
    const devtr  = new Discord.MessageEmbed() //#DevTR - xFalcon
        ...
        .addField(` **Kullanıcılar**`, `:green_circle: Çevrimiçi : **${message.guild.members.cache.filter(o => o.presence.status === 'online').size}** \n:red_circle: Rahatsız Etmeyin : **${message.guild.members.cache.filter(dnd => dnd.presence.status === 'dnd').size}** \n:crescent_moon: Boşta: **${message.guild.members.cache.filter(i => i.presence.status === 'idle').size}** \n:white_small_square: Görünmez/Çevrimdışı : **${message.guild.members.cache.filter(off => off.presence.status === 'offline').size}** \n:robot:  Botlar : **${botlar}**`, true) //#DevTR - xFalcon
        .setTitle(`${message.author.tag} - Tarafından Kullanıldı`) //#DevTR - xFalcon
        .setFooter(`AggSy Sunucu Sayımı`, client.user.avatarURL()) //#DevTR - xFalcon
